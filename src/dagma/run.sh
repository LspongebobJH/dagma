...
#             for seedX in "${seedsX[@]}"; do
#                 # PLS
#                 if [ $suffix = '_option_5_PLS_normX_sym1_grnboost2' ]; then
...
#                 elif [ $suffix = '_option_10_PLS_normX_sym1_grnboost2' ]; then
#                     CUDA_VISIBLE_DEVICES=${cuda_idx} \
#                     python knockoff_v44.py \
#                     --W_type=W_est \
#                     --data_version=v${dst_data_version} \
...
seedsX=( {1..3..1} )
for seedX in "${seedsX[@]}"; do
    for ntrees in "${ntrees_list[@]}"; do
        for max_feat in "${max_feat_list[@]}"; do
            for max_sample in "${max_sample_list[@]}"; do
                src_note="_normX_sym1"
                dst_note="_normX=sym1_ntrees=${ntrees}_max_feat=${max_feat}_max_sample=${max_sample}_grnboost2"
                ...
                python genie3.py \
                    ...
                    --disable_norm \
                    --force_save \
                    --ntrees=${ntrees} --max_feat=${max_feat} --max_sample=${max_sample} \
